...
class RecycleAdapter(zombieRecycler: ZombieRecycler):RecyclerView.Adapter<RecycleAdapter.ViewHolder>(){
    ...
    var context: Context
    init {
        this.context = zombieRecycler
    }

    val movieTitles =  arrayOf(
            "Night of the Comet","Dead Snow","Cemetery Man",
            "28 Weeks Later","Night of the Creeps","ParaNorman","Zombieland",
            "Planet Terror","Train to Busan","The Beyond","Day of the Dead",
            "The Serpent & the Rainbow", "Re-Animator","Zombi 2","Dawn of the Dead",
            "Return of the Living Dead","Dead Alive","28 Days Later","Night of the Living Dead",
            "Shawn of the Dead","Dawn of the Dead")

    val movieYears = arrayOf("1984","2009","2004","2007","1986","2012","2009","2007",
            "2016","1981","1985","1988","1985","1971","2004","1985","1992","2002",
            "1968","2004","1978")

    val movieDirectors = arrayOf("Thom Eberhardt","Tommy Wirkola", "Michele Soavi",
            "Juan Carlos Fresnadillo","Fred Dekker","Chris Butler, Sam Fell","Ruben Fleischer",
            "Robert Rodriguez","Sang-ho Yen","Lucio Fulci","George Romero","Wes Craven",
            "Stuart Gordon","Lucio Fulci","Zack Snyder","Dan O'Bannon","Peter Jackson",
            "Danny Boyle","George Romero","Edgar Wright","George Romero")

    val movieImage = arrayOf("http://collider.com/wp-content/uploads/2016/10/night-of-comet.jpg",
    ...
    inner class ViewHolder(itemView: View):RecyclerView.ViewHolder(itemView){
        ...
        var movie_year: TextView

        init{
            movie_title = itemView.findViewById(R.id.movie_title)
            movie_year = itemView.findViewById(R.id.movie_year)

            //now pass the movie info into view (with intent)
            itemView.setOnClickListener{
                v: View ->
                var position: Int = getAdapterPosition()

                val intent = Intent(context, ZombieView::class.java)
                intent.putExtra("Title", movieTitles[position])
                intent.putExtra("Year", movieYears[position])
                intent.putExtra("Director", movieDirectors[position])
                intent.putExtra("Image", movieImage[position])
                intent.putExtra("Description", movieDescription[position])
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, i:Int):ViewHolder {
        val v = LayoutInflater.from(viewGroup.context).inflate(R.layout.zombie_list,viewGroup,false)
        return ViewHolder(v)
    }

    override fun onBindViewHolder(viewHolder: ViewHolder, i:Int) {
        viewHolder.movie_title.text = movieTitles[i]
        viewHolder.movie_year.text = movieYears[i]
    }

    override fun getItemCount(): Int {
        return movieTitles.size
    }
}//end RecycleAdapter class